...
board-arrow knobs (`plan_lines` 0–3 + a `threats` toggle, ADR-0029) and a
provider/model picker (#214: the shared `components/ModelSelect.vue` — a
grouped select over `api.providers.list`'s enriched `models`, whose effective
default — own default → global default → own lowest-id → global lowest-id,
pure `lib/providers.ts` — is preselected and labeled "(default)"; picking a
non-default entry sends `provider` + `model`, the default sends neither) over
...
`multipv` form (plus the same `ModelSelect` picker, #214) over
...
spawn prompt's first turn, so `create` is a single `Spawn` frame). A
**per-session model choice** (#215, ADR-0046) rides the same seam: the `new`
frame's optional `provider`/`model` (both or neither) are validated against the
caller's composed catalog (`knows_provider`; unknown ⇒ the typed
`UnknownProvider` → an `unknown_provider` error frame, *before* anything
spawns — a failed pin would silently leave the session on the `EchoLlm` stub)
and parked as a **one-shot pending pin** the sentinel resolution consumes
(`take_pending_pin(user).or_else(default_for)`). Mid-conversation switching
needs no backend at all: the SPA sends the engine's wire-allowed
`InMsg::SetModel` through the relay, and the engine re-resolves, rebinds (at
turn end when one is live) and broadcasts `OutEvent::ModelChanged` — which is
persisted and replayed like any event, so the transcript fold derives the
current model last-wins from the stream (every session opens with one from the
session-start pin; no `agent_sessions` column). Users manage
...
and a usage footer, plus two `ModelSelect` pickers (#215): one by the composer
choosing a *new* conversation's model (null = default row), one in the header
showing the transcript's folded current model and sending `set_model` on
change (applied after the live reply, per the engine's stash semantics);
`ProvidersSettings` (Settings) manages the caller's
