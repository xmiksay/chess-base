...
import { emptyQuery, isEmptyQuery, toParams } from '../lib/headerQuery.js'
import { lineFen, moveToSan, replayLine } from '../lib/openingTree.js'
...
export const useSearchStore = defineStore('search', () => {
  ...
  async function runHeaderSearch() {
    ...
    try {
      ...
      headerError.value = String(e.message ?? e)
      results.value = []
  ...
  const line = ref([]) // SAN moves from the start position
  const tree = ref([]) // MoveStat rows for the current position
  const games = ref([]) // GameHit rows reaching the current position
  ...
  async function loadPosition() {
    ...
    try {
      ...
      explorerError.value = String(e.message ?? e)
      tree.value = []
      games.value = []
    } finally {
