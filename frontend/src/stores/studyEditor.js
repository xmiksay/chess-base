...
import { childWithSan, firstChild, getNode, lastMainlineId, sanPath } from '../lib/moveTree.js'
...
export const useStudyEditorStore = defineStore('studyEditor', () => {
  ...
  const atEnd = computed(() => firstChild(tree.value ?? { nodes: [] }, nodeId.value) == null)
   ...
   */
  async function playMove({ from, to, promotion = 'q' }) {
    let move
    try {
  ...
  async function annotate({ comment, nag } = {}, id = nodeId.value) {
