...
describe('settings store', () => {
  ...
  it('load() reconciles with the server and maps snake_case', async () => {
    api.settings.get.mockResolvedValue({ theme: 'light', board_theme: 'blue', default_database_id: 3 })
    const s = useSettingsStore()
    await s.load()
  ...
  it('load() keeps the mirror when the server is unreachable', async () => {
    ...
    api.settings.get.mockRejectedValue(new Error('offline'))
    const s = useSettingsStore()
    await s.load()
    expect(s.theme).toBe('dark')
  ...
  it('update() applies optimistically, mirrors, and persists snake_case', async () => {
    api.settings.set.mockResolvedValue({ theme: 'dark', board_theme: 'green' })
    const s = useSettingsStore()
  ...
  it('update() surfaces a rejected save as an error but keeps the optimistic value', async () => {
    api.settings.set.mockRejectedValue(new Error('database 9 is not available'))
    const s = useSettingsStore()
