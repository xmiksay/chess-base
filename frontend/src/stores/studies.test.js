...
describe('studies store', () => {
  ...
  it('refresh loads the list of summaries', async () => {
    api.studies.list.mockResolvedValue([{ id: 1, name: 'Sicilian', global: false }])
  ...
  it('open loads a study with its tree into current', async () => {
    const view = { id: 2, name: 'Ruy', tree: { root: 0, nodes: [{ children: [] }] } }
    api.studies.get.mockResolvedValue(view)
    const store = useStudiesStore()
    await store.open(2)
  ...
  it('importPgn opens the new study and refreshes the list', async () => {
    const study = { id: 3, name: 'Imported', tree: { root: 0, nodes: [] } }
    api.studies.importPgn.mockResolvedValue(study)
    api.studies.list.mockResolvedValue([study])
    const store = useStudiesStore()
  ...
  it('exportPgn returns the PGN string', async () => {
    api.studies.exportPgn.mockResolvedValue('1. e4 e5 *')
    const store = useStudiesStore()
  ...
  it('rename keeps current and the list summary in sync', async () => {
    const store = useStudiesStore()
    store.list = [{ id: 4, name: 'Old' }]
    store.current = { id: 4, name: 'Old' }
    api.studies.rename.mockResolvedValue({ id: 4, name: 'New' })
  ...
  it('remove drops the study and clears current when it was open', async () => {
    const store = useStudiesStore()
    store.list = [{ id: 5, name: 'Gone' }]
    store.current = { id: 5, name: 'Gone' }
    api.studies.remove.mockResolvedValue(null)
  ...
  it('surfaces failures on error and resets loading', async () => {
    api.studies.list.mockRejectedValue(new Error('boom'))
    const store = useStudiesStore()
