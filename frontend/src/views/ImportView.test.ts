...
describe('ImportView', () => {
  it('populates the target picker and defaults to the first collection', async () => {
    ...
    expect((wrapper.find('[data-test="target"]').element as HTMLSelectElement).value).toBe('1')
  })

  it('shows an empty-state hint when there are no collections', async () => {
    vi.mocked(api.databases.list).mockResolvedValueOnce([])
    const wrapper = mount(ImportView)
    await flushPromises()
  ...
  it('uploads a selected PGN file into the chosen collection', async () => {
    vi.mocked(api.import.uploadPgn).mockResolvedValue({ imported: 2 })
    const wrapper = mount(ImportView)
    await flushPromises()
