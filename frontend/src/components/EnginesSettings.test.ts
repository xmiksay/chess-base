...
beforeEach(() => {
  ...
  vi.mocked(api.engines.default).mockResolvedValue({ default: 'Stockfish' })
})
...
describe('EnginesSettings', () => {
  it('lists engines and marks the default', async () => {
    ...
    const checked = rows[0].find('input[type="radio"]').element as HTMLInputElement
    expect(checked.checked).toBe(true)
  })

  it('upserts a new engine including the runner field', async () => {
    vi.mocked(api.engines.upsert).mockResolvedValue({ name: 'Maia', path: '/usr/bin/lc0' })
    const wrapper = mount(EnginesSettings)
    await flushPromises()
  ...
  it('selecting a radio sets that engine as the default', async () => {
    vi.mocked(api.engines.setDefault).mockResolvedValue({ default: 'SF-Windows' })
    const wrapper = mount(EnginesSettings)
    await flushPromises()
  ...
  it('surfaces an API error message', async () => {
    vi.mocked(api.engines.list).mockRejectedValueOnce(new Error('admin privileges required'))
    const wrapper = mount(EnginesSettings)
    await flushPromises()
